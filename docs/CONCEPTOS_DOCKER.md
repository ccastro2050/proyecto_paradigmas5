...
RUN apt-get update && apt-get install -y --no-install-recommends curl gnupg2 unixodbc  && c…
WORKDIR /app
...
| `EXPOSE 8007` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15437 de su PC (`-p 15437:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8008"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8007"]
    ...
    NAV -->|"localhost:8007"| APIFACTURAS
    NAV -->|"localhost:8008"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8007<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13336"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15437"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11436"| SQLSERVER
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15437:5432"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-facturas:
    ...
    ports:
      - "8007:8007"                # http://localhost:8007/docs
    environment:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8007`, `localhost:15437`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas5_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas5_default --restart unless-stopped `
  -e "POSTGRES_DB=bdfacturas_postgres_local" `
  -e "POSTGRES_USER=paradigmas" `
  -e "POSTGRES_PASSWORD=paradigmas123" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15437:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. mariadb
docker run -d --name mariadb --network proyecto_paradigmas5_default --restart unless-stopped `
  ...
  -p 13336:3306 mariadb:11

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver
docker run -d --name sqlserver --network proyecto_paradigmas5_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11436:1433 mcr.microsoft.com/mssql/server:2022-latest

# 7. ESPERAR a que responda de verdad… mirándolo a ojo

# 8. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_paradigmas5_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
...
docker run -d --name api-facturas --network proyecto_paradigmas5_default --restart unless-stopped `
  ...
  -p 8007:8007 api-facturas uvicorn main:app --host 0.0.0.0 --port 8007 --reload

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_paradigmas5_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8007" `
  -e "CLAVE_SESION=Paradigmas123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8008:8008 front-flask
...
```yaml
URL_API: http://api-facturas:8007
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8007` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8007` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8007:8007`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8007`, el front
...
```powershell
docker network inspect proyecto_paradigmas5_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas5` produce
la red `proyecto_paradigmas5_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas5-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas5_default has active endpoints (name:"proyecto_paradigmas5-front-flask-1" …)
```
