...
> `uvicorn main:app --port 8007 --reload` (arrancar la API),
> `python pruebas\prueba_capas.py` (la prueba de capas) y los `curl` del
> smoke test (`curl http://localhost:8007/api/producto`). En el chat, la IA
...
| `uvicorn main:app --port 8007 --reload` · `python pruebas\prueba_capas.py` | `api_facturas\` (ahí vive el código) — primero `cd api_facturas` |
...
7. En mi máquina TAMBIÉN corre el proyecto clonado del curso con sus
   ...
   a. Publica los puertos del host con +100: en el docker-compose.yml la
      API va "8105:8007" y PostgreSQL va "15535:5432" (adentro de los
...
3. **El punto de control real es el smoke test final** (7_quickstart.md):
   ...
   (regla 7 del prompt) — donde el quickstart diga `localhost:8007` use
   `localhost:8105`, y donde diga `15437` use `15535`.
