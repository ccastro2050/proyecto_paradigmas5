...
#       empresa y vendedor) ──────────────────────────────────────────
curl http://localhost:8007/api/persona                       # 6 personas
curl http://localhost:8007/api/persona/P001                  # Ana Torres
curl -X POST http://localhost:8007/api/persona -H "Content-Type: application/json" `
     -d '{\"codigo\":\"P007\",\"nombre\":\"Prueba V2\",\"email\":\"p7@correo.com\",\"telefono\":\"3007777777\"}'
curl -i -X PUT   http://localhost:8007/api/persona/P007 -H "Content-Type: application/json" `
     -d '{\"telefono\":\"3008888888\"}'                      # 422: a PUT le faltan campos
curl -X PATCH http://localhost:8007/api/persona/P007 -H "Content-Type: application/json" `
     -d '{\"telefono\":\"3008888888\"}'                      # 200: PATCH acepta subconjunto
curl -X DELETE http://localhost:8007/api/persona/P007
curl -i -X DELETE http://localhost:8007/api/persona/P001     # 500: FK (P001 es cliente)

curl http://localhost:8007/api/cliente                       # 4 clientes (ids 1,2,3,5)
# cliente MÍNIMO: credito lo pone el DEFAULT de la BD, empresa queda null:
curl -X POST http://localhost:8007/api/cliente -H "Content-Type: application/json" `
     -d '{\"fkcodpersona\":\"P001\"}'
# la FK como última defensa:
curl -i -X POST http://localhost:8007/api/cliente -H "Content-Type: application/json" `
     -d '{\"fkcodpersona\":\"P999\"}'                        # 500 FK

# ── 3. LA CADENA COMERCIAL (criterio 3) ─────────────────────────────
curl -X POST http://localhost:8007/api/empresa -H "Content-Type: application/json" `
...
curl http://localhost:8007/api/cliente     # ← ANOTE el id del cliente de P010 (los
#     autonuméricos se consumen también en los inserts fallidos — research D5)
curl -X POST http://localhost:8007/api/vendedor -H "Content-Type: application/json" `
     -d '{\"carnet\":1004,\"direccion\":\"Calle 9 #8-70\",\"fkcodpersona\":\"P010\"}'
curl http://localhost:8007/api/vendedor    # ← anote el id (será 4)
# la factura de la cadena (use LOS IDS ANOTADOS) y anúlela al final del smoke.

# ── 4. FACTURA LEE POR SPs (criterio 4) ─────────────────────────────
curl http://localhost:8007/api/factura                       # 6 con nombres y productos adentro
curl http://localhost:8007/api/factura/1                     # una completa
curl -i http://localhost:8007/api/factura/999                # 404

# El stock ANTES:
curl http://localhost:8007/api/producto/PR001                # stock 17
curl http://localhost:8007/api/producto/PR003                # stock 42
# Crear (2 renglones — nadie envía subtotales):
curl -X POST http://localhost:8007/api/factura -H "Content-Type: application/json" `
...
curl http://localhost:8007/api/producto/PR001                # stock 15
curl http://localhost:8007/api/producto/PR003                # stock 39

# ── 5. ERRORES DE NEGOCIO (criterio 5) ──────────────────────────────
curl -i -X POST http://localhost:8007/api/factura -H "Content-Type: application/json" `
...
curl -X POST http://localhost:8007/api/factura/7/anular      # 200: restaura stock
curl http://localhost:8007/api/producto/PR001                # stock volvió a 17
curl -i -X POST http://localhost:8007/api/factura/7/anular   # 409 ya anulada
curl -i -X POST http://localhost:8007/api/factura/999/anular # 404
```
