...
```powershell
curl http://localhost:8007/     # → "version":"v3", "motor":"postgres"
```
...
curl http://localhost:8007/            # → "motor":"mariadb"
```
...
```powershell
curl -i http://localhost:8007/api/factura/999               # 404 "Factura 999 no existe"
...
| `mariadb` no queda healthy | Puerto 13336 ocupado, o volumen de un intento fallido: `docker compose down -v` y de nuevo |
