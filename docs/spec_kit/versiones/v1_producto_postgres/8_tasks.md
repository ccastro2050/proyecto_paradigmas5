...
- [ ] Crear el `docker-compose.yml` con el servicio `postgres` (volumen
      `pgdata`, `db/init.sql` montado, puerto 15437, healthcheck — ver
...
**Verificar:** `uvicorn main:app --port 8007 --reload` y en `/docs` probar:
...
- [ ] Agregar al `docker-compose.yml` el servicio `api-facturas`: `build:`,
      código montado como volumen + `--reload`, puerto 8007, `DB_POSTGRES`
